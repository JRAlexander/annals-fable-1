...
export function productionSystem(state: GameState, _out: SimEvent[]): void {
  for (const s of state.settlements) {
    ...
    // desired per job, capped by what the land offers; spill remainder in fixed order
    const assigned: Record<string, number> = {};
    let used = 0;
    for (const job of WORK_JOBS) {
      const want = Math.floor((workers * s.alloc[job]) / total);
      const got = Math.min(want, s.siteCapacity[job]);
      assigned[job] = got;
      used += got;
    }
    ...
    for (const job of WORK_JOBS) {
      if (leftover <= 0) break;
      const spare = s.siteCapacity[job] - assigned[job];
